...
**The deliverable is always copy-ready prompts** — never a description of a prompt. **In both run modes the agent also submits them through the §5 connectors and ships the media with them** (correction 2026-09-26, user: "in Manual we will always use connectors for generating"); the prompt is still written, labelled and delivered with every render.

**Run mode — Manual is the default, always (§44 default 83).** **Manual: the agent generates every image, video and voice through the §5 connectors itself** (Higgsfield images, Kling connector, ElevenLabs connector, HeyGen connector, Kie API for Seedance), checks each render (§22V, §22W), and **stops at the human gates** — the avatars (step 3), the hooks (step 6), the voice clone (§22U step 6, the connector has no clone call) and the voice master listen (§22U step 10) — plus the final review *(correction 2026-09-26, user; before this the user ran every generation after the avatars)*. **Automatic** (Appendix E0) is the one exception: only when the user explicitly calls it — "we will use automation" or an equally direct instruction — the gates are removed: the agent confirms the hooks, clones by API, picks the master, rerolls failures, trims dead air and ships the finished videos. It is never inferred, never offered as a switch mid-build, and never carried into the next build. The prompts are written, labelled and delivered exactly the same way in both modes.
...
- **Audio is a row, never a portrait frame** *(user's named change, 2026-09-26)*: every audio file (VO takes, clone sources) shows as a compact row with a player, its length, status, Confirm and Download. **VO shows one row per part** (HK1…, BODY): that part's takes are its versions, picked from the board's own version picker (each version with its length, pace and a Confirmed tag) and played in the board's own audio player (play/pause, a progress bar to click or drag, elapsed / total time; one plays at a time), laid out in fixed columns so every player is the same length whatever sits beside it); confirming a take locks the part and replaces any take confirmed before it.
- **Navigation in plain words** *(user's named change, 2026-09-26)*: the view tabs carry icons and a badge with how many items wait for the reviewer; statuses read Planned · Prompt ready · Generating · To check · Confirmed · Fixing everywhere, the status filter shows each with its colour and dims the empty ones; the Manual run groups are progress tiles (done of total, a bar, a ✓ when finished, a badge for what waits on the reviewer — VO counted per part); "Only what needs me" is an on/off switch.
- **One look across the three tabs** *(user's named change, 2026-09-26)*: Manual run and Plan use the Board's design — the numbered stage strip across the top (groups in Manual run, documents in Plan, each with its progress and a ✓ when done) and every group or section as a Board card (number badge, title, a one-line subtitle, progress bar), with Images and Videos as sections inside a group rather than boxes inside boxes.
- **Download goes straight to the file** *(user's named change, 2026-09-26)*: every Download button (card, viewer, audio row, zip) goes straight to saving the file. It never opens a new tab, a link or a menu of its own. The only step left is the platform's own Save confirmation, which a page can't skip.
- **The viewer stays in its group** *(user's named change, 2026-09-26)*: clicking an image or video opens the viewer on that card's group only. On the Board that is its stage card (Cast, Locations, B-roll…); in Manual run and Plan it is its act or group (Hook 1, Act 2…). The arrows and the filmstrip move within that group, and the counter names it ("2 / 6 images · Act 2").
- **Spare VO takes don't count as waiting** *(user's named change, 2026-09-26)*: once a VO part has its confirmed take, the other takes of that part stay as versions but are left out of the counts and the To check filter. Only work that really needs a check shows as To check.
- **Confirming moves the run on** *(user's named change, 2026-09-26)*: in Manual run, once every card in a group is confirmed (for example every cast image), the board says so and moves to the next group that still needs work (Cast → Locations → Voice → Hook 1 → Act 1 …). Clicking anywhere in the viewer's details panel, including the Fix box, never closes the viewer.
...
5. **On the user's go, the build continues in Manual:** steps 4–5 as one delivery, built against the approved avatars — **the agent generates the plates through the connectors (correction 2026-09-26)**, checks them and ships them with their prompts; the voice route is generated by the agent too, stopping for the clone in the ElevenLabs app (§22U step 6) and the master listen (step 10); the agent judges every render (§22V, §22W); hooks one by one at step 6, each approved by the user; then step 7 and the CapCut block.
...
**Manual vs Automatic.** In Manual the agent still fetches, measures and absorbs the inspo, script and Product Sheet itself, because absorption is analysis rather than generation, then generates the avatars (step 3) and stops for the user's decision on them (V7.62.0). Steps 4–5 and the voice route are then generated by the agent through the connectors (correction 2026-09-26), each render delivered with its prompt and verdict, stopping at the Manual gates. In Automatic (`RUN: AUTOMATION`), the agent generates every cast sheet, plate and voice itself, with the §19 panel check, the §30C/§30G plate checks and E1 on each one, and stops at the E0 stop points.
...
Both run modes (E0). **Manual:** the agent runs every step through the connectors and delivers each step's prompt, text and settings with its result; it stops at the clone (step 6 — the ElevenLabs connector has no clone call, so the user clones in the app) and at the master listen (step 10) *(correction 2026-09-26)*. **Automatic:** the agent runs everything, cloning by API and picking the master itself.
...
**Rules.** `split`, `pip` and punch-ins need a talking-head track: in a voice-only build they FAIL, never fall back silently. Every §30H rule holds for every layout — PLACE, JOIN, FLICKER, HOLE and FLASH. A punch-in landing under a B-roll is reported and ignored. **Measured V7.63.0 on a synthetic cut:** split, picture-in-picture (both ways), full-frame and a punch-in/punch-out rendered to duration with 0 black frames; each region's pixel colour sampled inside its band or box and matched its source; the face point held still through the punch-in. **Unverified:** production clips.
...
**83. Run mode → Manual (§1, Appendix E0).** The agent writes the prompts **and generates through the §5 connectors**; the user decides at the gates — avatars, hooks, clone, master listen, final review (correction 2026-09-26). **Automatic only on explicit instruction, per build** — "we will use automation" or an equally direct call — through the `ai-prompt-engineer-auto` skill. Never inferred from a request to "check", "review" or "fix" a render, never switched on mid-build without the call, never carried into the next build. Analysing or trimming a single clip the user supplies is a Manual task, not Automatic.
