...
**The one exception is §9B**, and only on a script line that explicitly concerns putting it on — or its fit (§9B scope).
...
**Fit and adjustability lines count as putting-on lines** *(V7.60.9)*. A line about fit — "adjustable", "fits any size", "one size fits all", "easy to put on" — is covered by a seating beat and/or the product itself (in hand, or worn and already seated), **never by the product being adjusted**: no band pulled tighter, no tail tugged, nothing threaded, opened or worked at the hardware. **The word "adjustable" never enters a prompt** — a generator renders it as the act of adjusting. The Product Sheet names the seating direction and the end-position reference.
...
# CHANGELOG — V7.60.8 → V7.60.9 *(cut authorised)*

| § | Change |
|---|---|
| **9B** | Scope: fit and adjustability lines count as putting-on lines — covered by a seating beat and/or the product itself, never by adjusting it; "adjustable" never enters a prompt |
| **9** | "Always already in place" exception points at the fit clause |

**Origin:** user check — B-roll on "adjustable" lines showed the strap being worked instead of being put on.
