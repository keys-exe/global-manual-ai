...
Work runs continuously. Steps 3, 4 and 5 **send** their artefact and continue in the same pass — a handoff, not an approval. **In Manual there are two gates: the absorption review after step 2 (V7.62.0) and step 6.** **In Automatic there is no gate: the agent confirms each hook itself (E0, V7.60.6).**
...
| 2 | **Absorb script, product, Product Sheet — and lock mode and model** | DET → **HG** *(Manual)* | The `.md` + `.py` pair, created where absent. §43A claims pass — every figure tiered before anything builds against it. **The phrase inventory is built here** (§27B): a mechanical pass over the script as written. **The Visual Instruction Ledger is opened here (§27F)** — every visual note on the script and every Loom instruction (§18C), each anchored to its spoken line. **The Mode & Model Lock is written here (§18A)** — mode per act, image model per beat class, read off what the script demands. Every other lock resolved here — camera, format, tools, mechanism claim, declared side. **Manual (V7.62.0): steps 1–2 ship as the absorption delivery and the build waits for the user's go before step 3.** Automatic runs straight on (E0) |
...
| 6 | **Hooks, one by one** | AC → **HG** | Serial: deliver → generate → first-frame check (§5/§30C/§30E) → confirm → next. Seeds built against plates that already exist. Confirmed renders feed the Scene and Subject Registries as they land. **The second gate** in Manual, after the step-2 absorption review; in Automatic the agent confirms (E0) |
...
**Manual: steps 1–2 ship as the absorption delivery and wait for the user's go; steps 3–5 then ship as one delivery, and nothing inside it waits (V7.62.0). Automatic: steps 1–5 run as one pass with no stop (E0).**
...
DET steps ship their checks with the deliverable — reconciliation lines, count tables, axis tables, QA matrices. In Manual the pipeline stops only after step 2 and at step 6.
...
**Everything steps 1 and 2 need arrives in one message** — the Intake Pack. The agent then runs steps 1–2 without a question. **In Manual it ships them as the absorption delivery and waits for the user's go before steps 3–5 (§18, V7.62.0)**; in Automatic it runs steps 1–5 as one pass and goes straight on to the voice route. The template lives at `builds/INTAKE_TEMPLATE.md`.
...
- **Each `ADJUST` line is applied, and recorded in the Build Sheet with the section it overrides.** An adjustment that contradicts a higher authority layer (§1: reference images, Product Sheet, locked standards) is **flagged at the absorption delivery (step 2) and not applied silently**. Everything else runs.

**The stops that remain in a one-message run — Manual:** the absorption review after step 2 (V7.62.0), hook approval (§18 step 6), the voice clone (§22U step 6), the voice master listen (§22U step 10), and the final review. **Automatic (V7.60.6): none.** The Drive link with `RUN: AUTOMATION` is the last message the run needs; the agent approves every step and delivers only the finished videos. `CAP` overrides the E0 default caps; `VOICE` absent → derived per §22D. The one exception is an intake that cannot be absorbed (no script, unreadable file) — that is reported before any credit is spent.

### The Manual Drive run *(new V7.62.0)*

**A Drive link with `RUN: MANUAL` (or `RUN` blank) is a full intake, not a request for prompts only.** The agent does the intake work itself, exactly as in Automatic, because fetching and absorbing is analysis, not generation:
...
3. **Ship steps 1–2 as the absorption delivery, then stop.** The user reviews it. A correction is applied (§34) before anything is built on it.
4. **On the user's go, the build continues as Manual always has:** steps 3–5 as one delivery of copy-ready prompts; the voice route as prompts and settings; the user generates and sends the renders back; the agent judges each one (§22V, §22W); hooks one by one at step 6; the voice clone in the ElevenLabs app (§22U step 6); then step 7 and the CapCut block.

**The agent submits no paid generation call in a Manual run.** Up to step 2, the only difference from Automatic is the stop at the end of it.
...
2. **Steps 1–2** — Absorption Sheet, Product Sheet (created where absent), claims pass, phrase inventory, **Visual Instruction Ledger (§27F)**, Mode & Model Lock with `MODE` as given. **Manual stops here for the user's go (V7.62.0).**
...
**Manual vs Automatic.** In Manual the agent still fetches, measures and absorbs the inspo, script and Product Sheet itself, because absorption is analysis rather than generation, and stops after step 2 for the user's go (V7.62.0). Steps 3–5 and the voice route are then delivered as copy-ready prompts, in the order the user runs them. In Automatic (`RUN: AUTOMATION`), the agent generates every cast sheet, plate and voice itself, with the §19 panel check, the §30C/§30G plate checks and E1 on each one, and stops at the E0 stop points.
...
**Delivery:** the Mode & Model Lock ships in the step-2 absorption delivery as a ledger widget — one row per beat class, the model and params, a status badge, and the step-2 read that justified it.
...
**85. Intake → one message, then no questions (§18B).** The Intake Pack carries steps 1 and 2; `MODE` locks the mode, `RUN: AUTOMATION` is the Automatic call, and the voice route follows the mode. **`RUN: MANUAL` (or blank): the agent fetches and absorbs steps 1–2 itself, stops for the user's go, then continues in Manual (V7.62.0).** **Film voices (Mode 4, 5, AI Drama) are neutral Seedance masters kept exactly as generated — never trimmed, sped or looped** (§24I).
...
**55. Build order → the eight-step flow (§18).** Steps 1–2 ship as the absorption delivery and, in Manual, wait for the user's go (V7.62.0); steps 3–5 then ship as one delivery with no stops inside it — steps 3, 4 and 5 send and continue. The other gate is the hooks. **The script is absorbed as written** — `CUT` is withdrawn, `BLOCKED` replaces it, and a line is never rewritten to rescue a claim or a coverage gap. The phrase inventory is built at step 2.
...
**Manual is the default** (§44 default 83). The agent writes copy-ready prompts; the user runs them and reviews. Nothing in this appendix executes. **A Drive intake with `RUN: MANUAL` is still fetched and absorbed by the agent (§18B, V7.62.0)** — that is analysis, not generation — and the run stops after step 2 for the user's go. On request, the agent may still fetch and check a render the user names by job ID or link, or trim a clip the user supplies (E11) — single tasks, not a run.
...
| § | Change |
|---|---|
| **18** | Manual has a second gate: steps 1–2 ship as the absorption delivery and wait for the user's go; steps 3–5 then ship as one delivery. Automatic unchanged — steps 1–5 in one pass |
| **18B** *(new sub-section)* | The Manual Drive run: `DRIVE` with `RUN: MANUAL` is fetched and absorbed by the agent exactly as in Automatic (`fetch_drive.py`, `fetch_loom.py`, steps 1–2 in full), then stops; on the user's go the build continues in Manual. No paid generation call in Manual. The absorption review joins the Manual stop list; `ADJUST` conflicts are flagged at the absorption delivery |
| **18A** | The Mode & Model Lock ships in the step-2 absorption delivery |
| **44** | Default 55: the absorption gate. Default 85: `RUN: MANUAL` named |
| **E0** | Manual: a Drive intake is still fetched and absorbed by the agent, then stops after step 2 |
| Files | `ai-prompt-engineer` skill (intake, build order), `ai-prompt-engineer-auto` skill (version), `builds/INTAKE_TEMPLATE.md`, `CLAUDE.md` (version) |

**Origin:** user instruction — "I will also use the Drive link and say run manual, just like the automation; you absorb them, and after that it's the same as what I'm doing." Absorption review chosen over a steps 1–5 opening delivery.
