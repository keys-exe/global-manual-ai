...
- **B-roll and Edit run full width** on the Board tab *(user's named change, 2026-09-26)*, edge to edge, one under the other.
- **Audio is a row, never a portrait frame** *(user's named change, 2026-09-26)*: every audio file (VO takes, clone sources) shows as a compact row with a player, its length, status, Confirm and Download. **VO shows one row per part** (HK1…, BODY): that part's takes are its versions, picked from a Version dropdown (take, length, pace, confirmed mark); confirming a take locks the part and replaces any take confirmed before it.
