...
- **The Plan tab** *(user's named change, 2026-09-26)*: beside Board and Manual run, the board shows the build's documents as they are written — the Absorption Sheet (step 1), the act map and the wardrobe map (step 5), and any other step document — one per rail entry, split into section tabs, tables rendered with the act map filterable by act and every beat id that is on the board opening its card in the viewer. The agent writes each document to the board the turn it writes or changes it; the repo file stays the source.
