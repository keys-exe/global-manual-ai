...
- **Audio is a row, never a portrait frame** *(user's named change, 2026-09-26)*: every audio file (VO takes, clone sources) shows as a compact row with a player, its length, status, Confirm and Download. **VO shows one row per part** (HK1…, BODY): that part's takes are its versions, picked from the board's own version picker (each version with its length, pace and a Confirmed tag) and played in the board's own audio player (play/pause, a progress bar to click or drag, elapsed / total time; one plays at a time); confirming a take locks the part and replaces any take confirmed before it.
