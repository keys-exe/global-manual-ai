...
Work runs continuously. Steps 3, 4 and 5 **send** their artefact and continue in the same pass — a handoff, not an approval. **In Manual there are two gates: the avatar review after step 3 (V7.62.0) and step 6.** **In Automatic there is no gate: the agent confirms each hook itself (E0, V7.60.6).**
...
| # | Step | Class | Carries |
|---|---|---|---|
| 1 | **Absorb the inspo video** | DET | The full §42 seven-part protocol → Absorption Sheet. Style Lock set, not proposed. Secondary references contribute named devices only, each recorded position-not-look. **The Edit Grammar (§42 Part 3A) is read here, off the shot frames and per-second sheets, and compiled into `EDIT-[BUILD]`** (V7.63.0) |
...
| 5 | **Act map and wardrobe map, together** | DET | Coverage ledger keyed to the step-2 inventory; **every Visual Instruction Ledger row assigned to the beat or CapCut line that carries it (§27F)**; **every B-roll row given its `layout`, and every punch-in, transition and caption device its line, from `EDIT-[BUILD]` (§42 Part 3A)**; **every B-roll row given its READ, `read_kind` and negation flag (§30B Part 4, V7.64.0)**; the §14A story-day derivation pass; wardrobe written per story day as beats land. Frame side, framing step, energy, valence, **story day, capture-event id**, six-slot rows, `GEO-LINE`s, recurring-subject IDs, product first appearance, claims. **Automatic (V7.60.7): written only after every step-3 sheet and step-4 plate has passed E1**, with identity strings and plate facts read off the renders; B-roll `duration` is `pending-master` until the voice master exists (E4, E6) |
...
**Manual: steps 1–3 ship as one delivery — the absorption plus the generated, checked avatars — and wait for the user's decision on the avatars; steps 4–5 then ship as one delivery, and nothing inside it waits (V7.62.0). Automatic: steps 1–5 run as one pass with no stop (E0).**

Three things that used to be gates and are not:

- **Step 3's render is a dependency, not an approval — in Automatic.** In Manual the agent still generates it, and the user then decides on the avatars (V7.62.0). The reference sheets — narrator, side cast and recurring anonymous subjects alike — are generated in-pipeline and the identity strings read off the result. It returns to the user only where generation is unavailable — and the strings are never taken from what was prompted as a substitute.
...
**Everything steps 1 and 2 need arrives in one message** — the Intake Pack. The agent then runs steps 1–2 without a question. **In Manual it then generates the avatars (step 3), ships steps 1–3 and waits for the user's decision on the avatars before steps 4–5 (§18, V7.62.0)**; in Automatic it runs steps 1–5 as one pass and goes straight on to the voice route. The template lives at `builds/INTAKE_TEMPLATE.md`.
...
### The Manual Drive run *(new V7.62.0)*

**A Drive link with `RUN: MANUAL` (or `RUN` blank) is a full intake, not a request for prompts only.** The agent does the intake and the avatars itself, exactly as in Automatic, then hands the build to the user:
...
5. **On the user's go, the build continues as Manual always has:** steps 4–5 as one delivery of copy-ready prompts, built against the approved avatars; the voice route as prompts and settings; the user generates and sends the renders back; the agent judges each one (§22V, §22W); hooks one by one at step 6; the voice clone in the ElevenLabs app (§22U step 6); then step 7 and the CapCut block.

**The avatars are the only paid generation the agent submits in a Manual run.** Plates, voices, hooks, B-roll and talking heads stay with the user.
...
| Loom brief *(optional)* | a video with `loom` in its name — the fallback when the `LOOM` link will not download (§18C). **Sorted before the inspo rule: it is never measured as an inspo** and `fetch_drive.py` runs `fetch_loom.py` on it | `.mp4` `.mov` `.webm` |
...
3. **Steps 3–5** — cast sheets (§19) — **in Manual the agent generates the cast sheets, then stops for the user's decision on the avatars (V7.62.0)** — Property Sheet and plate (§30G), Location Sheets and plates (§30C), act map and wardrobe map.
...
**Manual vs Automatic.** In Manual the agent still fetches, measures and absorbs the inspo, script and Product Sheet itself, because absorption is analysis rather than generation, then generates the avatars (step 3) and stops for the user's decision on them (V7.62.0). Steps 4–5 and the voice route are then delivered as copy-ready prompts, in the order the user runs them. In Automatic (`RUN: AUTOMATION`), the agent generates every cast sheet, plate and voice itself, with the §19 panel check, the §30C/§30G plate checks and E1 on each one, and stops at the E0 stop points.
...
|---|---|---|---|
| ~~TTS (narrated, §3A)~~ *(retired V7.60.5)* | ~~ElevenLabs library voice ID~~ | — | Replaced by Cloned: narrators come from the step-2 Kling clips like every other character (§22U) |
...
| **Cloned (default, §22U)** | ElevenLabs clone of the §22U step-2 source | Free — the clone's voice ID is the lock | Steered once: `VOICE-[CHAR]` goes into the step-2 Kling clips, and the clone inherits it |
...
A bookended build (§3A) has generated bookends and TTS narration — two sources, one character: the exact §22C two-voices failure, assembling late. Rule *(V7.60.5)*: **one Kling voice source (two or more takes) per character, one clone** — the bookends and the narration are both voiced from that clone through §22U, so there is nothing to match. No TTS audition.

**Casting gate (TTS regime)** — *retired V7.60.5.* Library voices are never auditioned or cast. The voice is cast in the §22U step-2 Kling clips through `VOICE-[CHAR]`; a voice that fails is re-rolled there, never replaced with an ElevenLabs voice.
...
**Every cloned voice starts as Kling clips, at least two generations (correction 2026-09-26, Pending Amendments; it was Seedance under V7.60.5).** A character's voice is designed in the step-2 Kling clips, and its audio is extracted there; the clone and every TTS line inherit it. The §24I film voice masters (Mode 4, 5, AI Drama) are not cloned and stay on Seedance. **Never ElevenLabs Voice Design** (`creative_design_voice`, text-to-voice), **never a library, premade or shared ElevenLabs voice**, in either run mode, for any character or narrator. ElevenLabs is used for two things only: cloning the step-5 source (step 6) and speaking the script in that clone (step 9). A cloned voice that did not come from the step-2 Kling clips is a failed delivery.
...
| # | Step | Tool | Rule |
|---|---|---|---|
| 1 | **Talking-head image** | T2I per §18A (Nano Banana on any beat with a person) | The §19 character, 9:16, 2k, composed as a talking-head frame (§22F). This image is both the Kling start image (step 2) and the HeyGen avatar image |
| 2 | **Voice source clips** — **at least two Kling generations**, 10s each, of the character talking | Kling connector, `kling-video-v3_0_omni`, §36 JSON, `image_1` = the step-1 image, `aspect_ratio: "9:16"`, `resolution: "1080p"`, `duration: 10`, `enable_audio: true`, `prefer_multi_shots: false`. One generation per call, never `imageCount` > 1 on one line | **G1** speaks the script's opening line, **G2** the next line (G3+ the lines after), each cut to the E6 10s budget (≤20 words brisk, ≤18 unhurried). Same image, same `VOICE-[CHAR]` verbatim first in `delivery` (§22D), same §28A delivery in every generation — **these clips are where the character's voice is designed; the clone inherits it.** **Same-voice gate:** every take must be the same person as G1 (pitch median within ±10%, `voice_source.py`, plus the ear: timbre, accent, age). A take that drifts is regenerated with the same prompt (two regenerations, then the user); it is never joined |
| 3 | **Trim** dead air and inhales, **each take** | E11 trim pass | Talking-head rules; no keep-list — the source must be speech only |
| 4 | **Speed ×1.2**, each take | `ffmpeg` `atempo=1.2` | Pitch preserved. Audio only is required |
...
**How the agent sees a clip — two passes** *(V7.64.0)*:

1. **The whole clip.** `scripts/contact_sheet.py <clip>` tiles evenly spaced frames, always including the true first and last frame, into one image. It also reports duration, resolution, 9:16, audio, frozen runs and black runs. `--full` keeps every frame at full resolution for zooming into hands, product and text. From it the agent reads the **key frame** (the clip time of the frame that shows the beat's READ, §30B Part 4) and the dead parts (standing before the action, settling after it).
2. **The on-screen window.** The viewer never sees the whole render. They see the part the edit uses, chosen by §30H WINDOW (`best_window.py`). `contact_sheet.py <clip> --from <in> --to <out>` tiles that window alone, and **the questions below are answered on it**. A clip that shows the line at 2.1s but is on screen from 0.0s to 0.9s shows nothing. Where the voice master does not exist yet, pass 2 runs at assembly.

The agent reads the sheets, plus full frames wherever a question needs detail.

### The questions, in order — the first NO is the verdict

1. **Does it show the line — on screen?** The READ (§30B Part 4) is visible **inside the on-screen window**, at a stated clip time (`key`), and early: in the window's first quarter, and never later than 0.5s into a window under 1.5s (§27A). The clip performs the beat's phrase and function (§27B, §30B), in the right emotional register (§30F), **and carries out every Visual Instruction Ledger row assigned to it (§27F)**.
   - **Negation lines** (`no slipping`, `without gripping the rail`, `too small to reach it`) show the opposite state, never the failure they name. A strap slipping on a "no slipping" line is a Q1 NO, however good the strap looks.
   - **Blind read** *(V7.64.0)*: the window sheet goes to a second agent that has not seen the script, with no filename, beat ID or line on it. That agent writes one sentence on what the viewer sees. If the sentence does not carry the READ, Q1 is NO: `Q1: blind read "<its sentence>" ≠ READ → <fix>`. It costs one read, no generation.
...
7. **Is there enough live footage for its slot?** The on-screen window is the best part of the clip (§30H WINDOW) and covers its slot at 1.0x, or at no slower than 0.8x. A clip too short is REGENERATE at a longer duration. **A best window that is more than 30% dead footage, or at rest at the cut, is REGENERATE with one continuous action that lasts the span (§27A)** — the clip has less action than its slot.

The verdict line and the fix rule are §22V's: `<BEAT-ID> · <clip> · window <in>–<out>s · key <t>s · USE` or `… · REGENERATE · Q<n>: <fault> → <fix>`. The window and key are written to the beat's board doc with the verdict (`read`, `readKind`, `key`, `window`, §16A). The locked page does not display them; showing them is a board change the user asks for.
...
### The on-screen time decides the shot — short beats, long beats *(new V7.64.0)*

A B-roll is generated at least 3s long (E6), but it is on screen only as long as its phrase. In a list that is often under a second ("arthritis, / worn cartilage / and meniscus"; "Adjustable. / No slipping. / No sores."). **The script's split stands (§27B). The shot adapts to the time it gets.** The time comes from `broll_spans.py` on the voice master, before the prompt is written (E6).

| On screen | The shot | Why |
|---|---|---|
| **Under 1.5s (`SHORT`)** | **A STATE: the READ visible in every frame.** The swollen knee already held in both hands; the strap already locked on a knee mid-stride. An event only if it is **already under way on frame one** and the READ lands in the first 0.5s | An event needs time to start and finish. Cut at 0.9s, only the start is left |
| **1.5–3s** | The arc as written — CONTINUING → one COMPLETING action → UNRESOLVED — with the completing action landing in the first half | The default |
| **Over 3s (`LONG`)** | **One continuous action that lasts the whole span.** A slow walk down the stairs; the strap drawn tight through the whole line. Never a quick action followed by standing | Otherwise the window picker (§30H WINDOW) has only dead footage to choose from |

**The READ lands early.** The key frame sits in the first quarter of the on-screen window, never later than 0.5s into a SHORT beat. The prompt says so: "already mid-step on the first frame", not "starts to walk".

**Negation lines show the opposite state.** A generator renders the noun it is given, so "no slipping" comes back as a strap slipping. The motion names only what is true (the strap staying flat through a deep bend), and the failure goes in the negatives (`no strap sliding, no strap rolling down`), never in the action. The same holds for `without`, `not`, `never`, `unlike` and `too small to` lines. `broll_spans.py` flags every one (`NEGATION`).
...
- **the visual column of a two-column script table** (V7.61.1). A `.docx` table whose header row names a voice column (`VO`, `Voiceover`, `Voice`, `Audio`, `Script`, `Dialogue`, `Narration`, `Copy`) and a visual column (`Visual`, `B-roll`, `Shot`, `Scene`, `Video`, `On screen`, `Direction`, `Notes`) is read row by row: each row's visual cell is a note anchored to that row's spoken line. In a table without such a header, a note anchors to the spoken line in its own row.
...
**Hold length rides the line — the READ** *(V7.64.0; replaces the hold estimate and the cover window)*. Every B-roll beat carries on the act map:
- **READ** — the one thing the viewer must take from the shot, in one line: a noun, an action or a feeling ("the strap stays flat through the bend"). Written **before** the subject is picked, and the subject is picked to show it. A beat with no READ is not prompted.
- **`read_kind`** — `state` (visible in every frame), `event` (one action has to land) or `contrast` (two states: before/after, side-by-side). It sets the beat's FLASH floor (§30H), and with the on-screen time it sets the shot's shape (§27A).
- **negation** — yes when the phrase says what does not happen. The shot shows the opposite state (§27A).

After the render the row gains **`key`** (the clip time of the frame that shows the READ, read off the whole-clip sheet, §22W) and **`window`** (the in/out the edit uses, §30H WINDOW). Timing comes from the voice master (E6), never a words-per-second estimate.
...
**Manual: steps 1–3 are one delivery and stop for the user's decision on the avatars; steps 4–5 are the next delivery; the build then stops at step 6 (V7.62.0). Automatic never stops (E0).**
...
5. **WINDOW — the best part of the clip, never frame 0 by default** *(V7.64.0; user correction 2026-09-26)*. The on-screen window is chosen from the whole generated clip: the part that shows the READ. The dead part is cut, whether it is the start, the end or both. A walk down the stairs whose first 1.8s is the subject standing uses the step, not the standing.
   - `assemble.py` picks it with `best_window.py` (`"in": "auto"`, the default). Every window of the slot's length is ranked by action. It must still be moving at the cut (§27A), and the key frame must sit inside it and early.
   - The agent confirms the pick on the window sheet (§22W pass 2). A hand-picked `in` overrides it. A key frame that is off screen is a FAIL.
   - The in/out, the head and tail cut and the key frame go into the EDL, the editor notes (§40) and the beat's board doc (§22W).
6. **FLASH — none.** A B-roll on screen for less than its read floor is too short to read. The floors are **state 0.8s · event 1.2s · contrast 1.8s** (*estimates, unverified — to be measured on production cuts*). The on-screen time counts the pause after the phrase, as the edit does.
   - `broll_spans.py` catches it before generation (E6).
   - An event or contrast under its floor is re-planned as a state shot (§27A).
   - A state under 0.8s is merged into its neighbour's slot. In Manual that is flagged to the user first: the script's split is theirs.

### The instrument

`scripts/assemble.py <plan.json>` takes the master, the verbatim lines, the talking-head track (or none) and the B-roll list with each clip's phrase. It places, joins, closes flickers and holes, picks each B-roll's window (`best_window.py`, rule 5), reports every fix and every failure, and renders a 1080×1920 rough cut with the master as the only audio. It then **verifies the render**: duration equals the master to within two frames, and there are no black frames. A plan with any failure does not render. **Measured V7.60.0 on a synthetic cut:** four B-rolls placed on their phrases, including a numeral-vs-word match. Two flickers closed (1.47s and 0.73s, by 0.88x and 0.87x slow-down). Every cut landed frame-exact: pixel colour sampled at ±1 frame of each cut. Duration matched, with 0 black frames. The voice-only variant of the same plan correctly failed with three holes and two NEED_LONGER. **Measured V7.64.0 on a synthetic cut (WINDOW and FLASH):**
- A clip still for 0–1.8s and moving for 1.8–3.0s, on a 0.93s slot, was given the window 1.93–2.87s, so the standing was cut. The object's position sampled in the render at 2.62, 3.0 and 3.5s read x = 180, 398 and 720 against 192, 420 and 720 expected (within one frame). Frame-0 placement would have shown the still subject.
- A clip that moves and then settles kept its start (in 0.1s).
- A key frame at 1.5s in a 2.6s slot gave the window 1.0–3.6s, with the key 0.5s in.
- A hand-set `in` that left the key off screen FAILED (KEY_OFF_SCREEN). An `event` read on a 0.87s slot FAILED (FLASH) and passed as a `state`.
- Two hook variants PASSED, with body cuts and windows identical.

**Unverified:** the motion ranking on production clips. Handheld shake raises every window's motion, which is why the agent confirms each pick on the window sheet.
...
| `split` | B-roll in one band, the talking head in the other, cropped around the face | `broll_pos` top/bottom · `ratio` 0.3–0.7 of the height |
| `pip` | a box over the frame — the B-roll over the talking head, or the talking head over the B-roll | `over` th/broll · `corner` tl/tr/bl/br · `scale` 0.2–0.5 of the width · `border` px |
| punch-in | the talking head cuts in on a phrase's first word and holds until the next B-roll or the next punch-in; `scale` 1.0 punches back out | `punch_in`: phrase + `scale` 1.05–1.5 |

`th_focus_y` (default 0.4) is the face's height in the talking-head frame. The split crop centres on it, and a punch-in zooms around it so the face holds still.

**Rules.** `split`, `pip` and punch-ins need a talking-head track: in a voice-only build they FAIL, never fall back silently. Every §30H rule holds for every layout — PLACE, JOIN, FLICKER, HOLE, WINDOW and FLASH. A punch-in landing under a B-roll is reported and ignored. **Measured V7.63.0 on a synthetic cut:** split, picture-in-picture (both ways), full-frame and a punch-in/punch-out rendered to duration with 0 black frames; each region's pixel colour sampled inside its band or box and matched its source; the face point held still through the punch-in. **Unverified:** production clips.

### What the agent edits, and what CapCut still does

The agent delivers the **rough cut** (`08_EDIT`), its **EDL** (every clip, in/out, speed, phrase, fix) and the verification report. After the render, it reads `contact_sheet.py` of the rough cut plus a frame on each side of every cut, and judges the whole edit by §22W. CapCut still does captions, motion graphics, the ambient bed, music and supplied-asset cut-ins (§17, §17A), **and every `EDIT-[BUILD]` device the rough cut does not render (§42 Part 3A)**, working from the rough cut instead of from loose clips. **Every Visual Instruction Ledger row carried by the edit (§27F) is placed on its line** — in the rough cut where the instrument can carry it, else as a CapCut line with its ledger ID. §28G's designed-silence list and J-cuts stay CapCut lines.
...
- **Layout (§42 Part 3A, V7.63.0):** a B-roll whose act-map row is not `full` is framed for its crop in `camera.framing` — `split`: the action sits in the middle band of the frame, nothing that matters in the top or bottom quarter; `pip`: one subject, large and centred, readable at a third of the width. The clip is still generated 9:16
...
**B-roll windows (§30H WINDOW, V7.64.0).** Every B-roll's window: which seconds of the clip the edit uses (`BR-07 · use 1.93–2.87s · key 2.20s`), so a hand edit cuts the dead part, not the action.
...
| Shot frames — one just after each cut, one mid-shot *(V7.63.0)* | The Edit Grammar (Part 3A): how each shot is laid out, read off the frame, never remembered |
| Per-second contact sheets *(V7.63.0)* | Layout changes that are not cuts — a split-screen or picture-in-picture appearing over a held shot changes only part of the frame and **does not score as a scene cut** (measured V7.63.0) |
...
A timestamped beat table: t-in/t-out per shot, type (TH / BR / mechanism / product / proof / CTA), subject class, **§30B function**, register, overlay text if any, **and layout (Part 3A)**. The map is written in the same six-slot language the act map uses, so the act map falls out of the absorption instead of being re-derived from memory — **absorption output IS act-map input.**
...
### Part 3A — Edit Grammar: copy how the winner edits *(new V7.63.0)*

**The edit is style, and it is copied like the rest of the Style Lock.** Part 1 measures how often the reference cuts; Part 3A records **what each cut looks like** — how the B-roll is put on screen, how the talking head moves between B-rolls, how the cut is made, how the words appear. It is read shot by shot off the `fetch_inspo.py` frames and per-second contact sheets, never from memory of a watch.

**The Edit Grammar Sheet** — one row per device the reference uses, each with its ID (`EG01`, `EG02`…), the shots where it appears (`S03, S07 @ 0:12, 0:31`), **when it is used** (which kind of line: proof, stat, mechanism, story beat, CTA) and its measured parameters:

| Axis | What is recorded |
|---|---|
| **B-roll layout** | `full` — B-roll fills the frame · `split` — B-roll in one band, talking head in the other: which band, and the ratio · `pip` — a box over the frame: which is in the box (B-roll over the talking head, or the talking head over the B-roll), the corner, the box width as a share of the frame, the border · `cutout` — the creator cut out over the B-roll (green-screen style) · `card` — B-roll behind a text card |
| **Talking-head moves** | punch-ins and punch-outs (scale, and on what kind of line), jump cuts inside a take, reframes |
| **Transitions** | hard cut, zoom, whip, flash, glitch — and where each is used |
| **Speed** | speed ramps, freeze frames |
| **Captions** | position, words per line, case, highlight (per word, per line), emoji — style only; the words are always ours |
...
2. **Prompts (§35).** A B-roll is framed for its layout before it is generated — a split band or a picture-in-picture box crops the clip, so the action is placed where the crop keeps it. Every clip is still generated 9:16.
3. **Assembly (§30H).** `assemble.py` renders `full`, `split`, `pip` and punch-ins itself. **`cutout`, `card`, transitions other than the hard cut, speed ramps, captions, text overlays and SFX are CapCut lines** (§40) with their `EG` IDs — in both modes.
4. **Manual:** every layout and device is on its beat in the CapCut block, so the user's edit copies the reference; where the user sends the clips back, the agent may render the rough cut with `assemble.py` as in Automatic. **Automatic:** the rough cut and every hook variant are rendered with the layouts and punch-ins; the rest of the grammar goes to CapCut with the finished videos.
5. **Checked.** The rough cut's contact sheet (§22W) is read against `EDIT-[BUILD]`: every layout on its line, nothing the reference never does.

**Where the reference has several inspos,** the primary one sets `EDIT-[BUILD]`; a secondary one contributes a named device only, recorded position-not-look (§18 step 1).
...
**88. B-roll window → the best part of the clip (§30H WINDOW, §22W).** Never frame 0 by default: picked by `best_window.py`, confirmed by the agent on the window sheet, judged on the window. Every B-roll beat has a READ, `read_kind` and negation flag; FLASH floors state 0.8s · event 1.2s · contrast 1.8s, checked on the master before any B-roll call (`broll_spans.py`, E6).
...
**55. Build order → the eight-step flow (§18).** In Manual, steps 1–3 ship as one delivery — absorption plus the agent-generated avatars — and wait for the user's decision on the avatars (V7.62.0); steps 4–5 then ship as one delivery with no stops inside it. The other gate is the hooks. Automatic: steps 1–5 in one pass. **The script is absorbed as written** — `CUT` is withdrawn, `BLOCKED` replaces it, and a line is never rewritten to rescue a claim or a coverage gap. The phrase inventory is built at step 2.
...
5. **Act map** — beat IDs, register per act, retention beats, product first appearance, **frame side and framing step per beat** (§30A), **location per beat** (§22A), **ownership, function, subject class, alibi and energy class per B-roll beat, and run shape per beat group** (§30B), **READ, read kind and negation flag per B-roll beat, and its key frame and window once rendered** (§30B Part 4, §30H), claims flagged (§43A)
...
**Manual is the default** (§44 default 83). The agent writes copy-ready prompts; the user runs them and reviews. Nothing in this appendix executes, with one exception: **a Drive intake with `RUN: MANUAL` is fetched and absorbed by the agent, which then generates and checks the avatars (§18 step 3) and stops — the avatars are the user's decision (§18B, V7.62.0).** On request, the agent may still fetch and check a render the user names by job ID or link, or trim a clip the user supplies (E11) — single tasks, not a run.
...
| Clip verdict (§22W) | `contact_sheet.py` whole-clip sheet (key frame) + window sheet (`--from/--to`) + full frames where needed; blind read by a second agent | all seven §22W questions YES on the on-screen window; blind read carries the READ | AUTO (agent) | REGENERATE with the named fix; third failure of one fault → user |
...
| Holes and flicker (§30H) | `assemble.py` timeline | no TH window < 1.5s between B-rolls; voice-only: no uncovered frame; no B-roll under its read floor (state 0.8s · event 1.2s · contrast 1.8s) | AUTO | Extend → slow ≥ 0.8x → regenerate longer |
| B-roll window (§30H WINDOW) | `best_window.py` via `assemble.py`; window sheet | window = the best part (not frame 0 by default); key frame on screen; moving at the cut; ≤ 30% dead | AUTO (agent confirms) | Re-pick by hand; a clip with no live window → REGENERATE, one continuous action (§27A) |
| B-roll timing (E6) | `broll_spans.py` on the voice master | every phrase found; no FLASH; every SHORT / LONG / NEGATION / NO_READ flag acted on in the prompt | AUTO | Re-plan the shot as a state; merge a state under 0.8s (Manual: flag to the user) |
...
**`layout` (V7.63.0).** Every B-roll row carries `layout` from `EDIT-[BUILD]` (§42 Part 3A) — `full`, `split`, `pip`, `cutout` or `card`, with its fields — and every row carries the `EG` IDs of the devices on its line. `assemble.py` reads `full`, `split` and `pip`; the rest go to CapCut.

**`read` · `read_kind` · `negation` · `key` · `window` (V7.64.0).** Every B-roll row carries its READ, `read_kind` and negation flag from step 5 (§30B Part 4). After the render it gains `key` (the clip time of the READ frame) and `window` (in/out, head and tail cut) from §30H WINDOW. `assemble.py` reads `read_kind`, `key` and `in`; `broll_spans.py` reads `read` and `read_kind`.
...
- **Timed before the prompt (V7.64.0).** `scripts/broll_spans.py <spans.json>` reads every B-roll phrase off the master. It returns the phrase's span, its on-screen time (the pause after the phrase counts, and flickers are closed as the edit will close them) and its call length, and it flags `FLASH`, `SHORT`, `LONG`, `SPLIT`, `NEGATION` and `NO_READ`. Every flag is acted on in the prompt (§27A) before the call, and a FLASH is resolved before any B-roll is generated.
...
| § | Change |
|---|---|
| **30H** *(new rule 5, WINDOW)* | The best part of each B-roll clip is used, never frame 0 by default. `assemble.py` picks the on-screen window (`"in": "auto"`, the default) with `best_window.py`: ranked by action, still moving at the cut, key frame inside and early. The agent confirms the pick on the window sheet; a hand-set `in` overrides it; a key frame off screen FAILS. In/out, head and tail cut and key frame go in the EDL, the editor notes and the beat's board doc |
| **30H** *(rule 6, FLASH)* | Read floors by kind: state 0.8s · event 1.2s · contrast 1.8s (unverified), counting the pause after the phrase; caught before generation; the script's split is kept (a state under 0.8s is merged, flagged to the user in Manual) |
| **22W** | Two passes: the whole clip (key frame, dead parts), then the on-screen window (`contact_sheet.py --from/--to`), where the questions are answered. Q1: the READ on screen, early; negation lines show the opposite state; blind read by a second agent that has not seen the script. Q7: live footage; a best window > 30% dead or at rest at the cut → REGENERATE with one continuous action. The verdict line carries window and key |
| **27A** | The on-screen time decides the shot: under 1.5s a STATE with the READ in every frame; 1.5–3s the arc with the completing action in the first half; over 3s one continuous action lasting the span. The READ lands early. Negation lines: the motion names only what is true, the failure goes in the negatives |
| **30B** *(Part 4)* | READ, `read_kind` and negation flag per B-roll beat replace the hold estimate and the cover window; `key` and `window` added after the render |
| **18** | Step 5 writes READ, `read_kind`, negation; step 7 runs `broll_spans.py` before any B-roll prompt |
| **E6** | `broll_spans.py`: span, on-screen time and call length per beat from the master; FLASH, SHORT, LONG, SPLIT, NEGATION, NO_READ flags acted on before the call |
| **E1** | Clip verdict on the window + blind read; read floors; new rows for the B-roll window and B-roll timing |
| **E4** | `read · read_kind · negation · key · window` on the act-map row |
| **40** | Every B-roll's window as an editor line (`use 1.93–2.87s · key 2.20s`) |
| **44** | Default 88 |
| **Appendix C** | Act map carries READ, read kind, negation, key frame and window |
| Files | `scripts/best_window.py` (new), `scripts/broll_spans.py` (new), `scripts/contact_sheet.py` (`--from/--to`), `scripts/assemble.py` (WINDOW, read floors, `words` input), `scripts/variants.py` (windows checked identical across variants, `words` passed through), both skills |

**Origin:** user instruction — "how can we improve the checking if it's the best B-roll for that line, and what if the line was said so quick that the B-roll just flashes"; user correction — "check what part of the B-roll is much better to use… going down the stairs but the first part is just standing, so that part should be the one cut". The user's split example (28 beats, ten under ~1.2s spoken, estimated) set the short-beat rule: the split stays, the shot becomes a state. **Measured:** §30H, on a synthetic cut.

**Also cut at this version** — locked corrections written in on 2026-09-26 and carried in Pending Amendments until now:
