...
| 2026-09-26 | **Kling talking heads: 12s maximum per clip, and every gesture carries the meaning of its words.** (1) A Kling talking-head clip is never longer than **12s**; a longer run of speech is split at sentence ends into clips of 12s or less (E6 still sets each clip to its lines' span). (2) Gestures are never generic ("small natural hand gestures", "raises his hands"): every gesture is a §28B landing, named **"on the exact words '…'"**, with its shape taken from the §28C lexicon **by what those words mean** and passing the swap test. Up to 3 landings per clip (§28B); where the words point below frame (a knee, a foot) the shape is mimed at chest height (§28C). The same holds for a Kling talking head voiced through an Element (the character's voice sample bound to the Element), which is under test on down-forwards-again. | §28B, §28C, §29, E6; §4 talking-head row | pending |
