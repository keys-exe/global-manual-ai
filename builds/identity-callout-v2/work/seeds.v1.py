...
add("B19", "nano_banana_pro", ["S1-CAROL", "PLATE-PROPERTY"], person_seed(subj="S1", framing="WIDE", day="D3", interior=True, face=True, mood="pos",
  prose=candid("a phone propped on the stairs' third step, left recording", "low, looking along the hall toward the front door",
    "Carol, sixty-four, stocky, in a cream cable-knit jumper and black straight-leg trousers, walking briskly down the hall toward the front door with her handbag on her shoulder, reaching for the door latch, looking at the door, relaxed",
...
add("B21", "nano_banana_pro", ["front", "worn_front"], " ".join([A("CAM-LOCK"), A("FRAME-SCALE").replace("[SCALE]", SCALE["WIDE"]), A("FRAME-WIDE"),
  candid("a phone propped on a bin by the kerb of a British high street", "at waist height, looking up the pavement",
    "one-off passer-by: " + S4 + ", walking briskly toward the phone among Saturday shoppers, the black strap on his right knee, looking ahead, mouth shut",
  ...
  + "\n\nNegative: " + ", ".join([A("NEG-FILE"), A("NEG-FRAME"), A("NEG-BODY"), A("NEG-STAGED"), PS("NEG-PLACE"), PS("NEG-ORIENT"), A("NEG-MOOD"), A("NEG-SKIN"), A("NEG-M1"), "no readable shop signs, no brand names, no logos"]))
...
add("B25", "nano_banana_pro", ["S1-CAROL", "PLATE-LIVING", "front", "worn_bent"], person_seed(subj="S1", framing="CLOSE", day="D2", interior=True, mood="pos",
  prose=SEAT_PROSE("at knee height, forty centimetres away, three-quarter from her right") + " " + PS("SEAT-LOCK").split(" Both hands")[0] + " " + LIV_LIGHT, seat=True))
add("B26", "nano_banana_pro", ["S1-CAROL", "PLATE-PROPERTY", "front", "worn_bent"], person_seed(subj="S1", framing="WIDE", day="D2", interior=True, mood="pos",
  prose=candid("a phone propped on the top landing, left recording", "at floor level, tilted down the flight",
    ...
    "Shoes by the bottom step, the coat on the newel post, the front door behind her") + " " + STAIR_GEO_DOWN + " " + HALL_LIGHT,
  worn=["PLACE-BENT", "ORIENT-C"]))
