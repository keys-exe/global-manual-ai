...
- Manual run view: per stage, one **Images** box holds every image and one **Videos** box every video (cast and locations: images only; absorption, voice, edit: one Files box). Each card has **Confirm** and **Fix**. Fix opens "What should be fixed?"; pressing **Confirm & regenerate** sets `imageStatus` / `status` to `regenerate` with the note in `imageFault` / `fault`. When you pick up a `regenerate` card, treat that note as the user's fix request: regenerate with it, upload the new render and set the step back to `review`. A video unlocks once its image is `confirmed`.
