...
This repo runs under the **AI Prompt Engineer — Global Standards** (currently V7.64.0).
...
- One `generations/<build-id>__<BEAT>` doc per beat: `build`, `act` (`Hook 1`… for hooks, `Act 1`, `Act 2`… for talking heads and B-roll, set from the step-5 act map), `stage` (`absorption` `cast` `locations` `voice` `vo` `talking` `hooks` `broll` `edit`), `beat`, `title`, `line` (verbatim script line), `imagePrompt` / `imageModel` (step 1), `prompt` / `model` (step 2 video, or the only prompt), `duration`, `credits`, `status` (`planned` `ready` `generating` `review` `use` `regenerate`), `imageStatus` (`review` `confirmed` `regenerate`), `fault` / `imageFault` (`Q<n>: fault → fix`), `regens` / `imageRegens`, `imageAsset` / `imageType`, `videoAsset` / `videoType`, and per step the connector record: `imageConnector` / `videoConnector` (Higgsfield, Kling, Kie AI…), `imageUrl` / `videoUrl` (the connector's original link), `imageCredits` / `credits`, `imageRes` (e.g. 1080×1920) / `duration` (s), `imageAt` / `videoAt` (ms); `updatedAt` (ms). B-roll beats also carry `read` (the one thing the viewer must see), `readKind` (`state` `event` `contrast`), `negation` (true/false), and after the render `key` (clip time of the READ frame, s) and `window` ([in, out] s, the part the edit uses) (§30B Part 4, §30H WINDOW; stored on the doc — the locked page doesn't display them).
- **Plan documents go on the board too** (the Plan tab). The turn you write or change the Absorption Sheet (step 1), the act map or the wardrobe map (step 5), write it to `docs/absorption`, `docs/actmap` or `docs/wardrobe`: `{title, step, order, source, build, updatedAt, sections:[{title, md}]}` — one section per `###` heading of the repo file, markdown kept as written (tables render; a table with an `Act` column gets act filters; first-column beat ids that match a card become links). Other step documents may be added the same way (`order` sets the rail order). The repo file stays the source. When the user approves the absorption, set `confirmed: true` on `docs/absorption` (the stage then shows green ✓; it also does once `docs/actmap` exists).
- **Voice, VO, Talking heads.** On the build doc set `kind` (`narration` or `film`), `talkingHeads` (true/false), and `voices`: `[{id, name, needed, status: missing|building|cloned|locked, voice, note}]` — narration: the narrator and any talking-head characters are `needed`; film: every speaking character. VO takes use `stage: "vo"` (beat `VO-T<n>-<PART>`, part HK1…, BODY); confirming one take per part locks the VO (or set `voLocked: true`). Talking heads only when the build has them; films have no VO and no Talking heads stage.
- **Keep every version.** Before a new render replaces a step's current one, the step's `imageVersions` / `videoVersions` array keeps every render: `{v, asset, parts, type, url, model, connector, credits, size, at, note}` (`note` = the Fix note that produced it). Append the new render as the next `v` and point the step's own fields at it. Never delete an old version.
