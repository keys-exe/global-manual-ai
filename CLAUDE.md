...
- Write with the `ArtifactData` tool (batch when more than two docs). When you write a prompt, add or update its doc (`status: ready`). When a render exists, download it locally, upload it with the Artifact tool (`url` = board, `asset: true`) and store the returned id in `imageAsset` / `videoAsset`. **Manual: the user checks every generation** — every new render goes on as `review` (To check); never confirm, reject or regenerate one on your own judgement, only from the user's Confirm or Fix. Automatic only: record the §22V / §22W verdict the same turn.
