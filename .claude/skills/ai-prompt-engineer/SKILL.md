...
- `scripts/vo_trim.py` — VO trim for audio-only §22U masters/hooks, in the **house cut** measured from the user's reference edit (`references/vo_house_cut.md`, Pending Amendment 2026-09-26): butt joins (0.015s at phrase ends), words kept to −38 dB, inhales cut at phrase boundaries (`--script` lines); hook variants = raw hook + raw body trimmed in one pass; flags a take whose last word the TTS cut off
