...
description: AUTOMATIC run mode of the AI Prompt Engineer Global Standards (V7.64.0, Appendix E0/E11, §5, §18B, §22U, §22V, §22W, §24I, §30H). Load ONLY when the user explicitly says "we will use automation", sends an Intake Pack or Drive intake with RUN: AUTOMATION, or directly instructs you to run a build automatically (generate, check, reroll and trim yourself). Never load it for ordinary prompt-writing, for "check this render", "fix this" or "trim this clip" — those are the default Manual mode (ai-prompt-engineer). Requires ai-prompt-engineer loaded too.
---
...
5. **Intake → steps 1–5 (§18B).** Drive: `python3 .claude/skills/ai-prompt-engineer/scripts/fetch_drive.py <BUILD> <folder link>`; links: `fetch_inspo.py <BUILD> <links…>`. **Loom (§18C, optional):** `fetch_loom.py <BUILD> <LOOM link>` — read `loom.md` and the frames; a Loom that will not download is listed in *Flags* and the run continues without it. A missing script, unreadable document or FETCH_FAILED link that blocks absorption is reported before any credit is spent — the one message allowed before the final delivery. Then run **in this order — each stage finished and passed before the next starts** (E0 run order, V7.60.7):
   ...
   4. **Act map + wardrobe map** (§18 step 5) — only now, built from what the sheets and plates rendered. B-roll `duration` = `pending-master` (E4); every B-roll row gets its `layout` from `EDIT-[BUILD]`.
...
5. **QA per E1.**
   ...
   - **Video: you are the judge (§22W).** Pass 1: `contact_sheet.py <clip>` → Read the sheet (true first and last frames) and note the key frame (the READ) and the dead parts; `--full` and Read single frames where hands, product or text need detail. Pass 2 (V7.64.0): `best_window.py <clip> --len <on-screen s> --key <t> --sheet` → Read the window sheet and answer the seven questions **on the window** — line (READ on screen, early; negation shows the opposite state; blind read: give the window sheet alone to a subagent that hasn't seen the script, and its one-sentence description must name the READ), product every frame, body every frame, motion, continuity, technical, enough live footage → `<BEAT> · window <in>–<out>s · key <t>s · USE` or `REGENERATE`. Silence and timing checks run on the audio (`silencedetect=noise=-40dB:d=0.4`).
...
9. **Assemble (§30H)** once an act's B-rolls pass: write `plan.json` (master, `script.lines.txt`, talking-head track or `null`, each B-roll with its phrase, `layout`, `read`, `read_kind` and `key` (`in` left `auto` unless you hand-picked it), the `punch_in` list and `th_focus_y` from `EDIT-[BUILD]`) → `assemble.py plan.json --out builds/<build>/edit/<act>.mp4`. Every FAIL is fixed (NEED_LONGER → regenerate that clip longer; KEY_OFF_SCREEN → re-pick the window or regenerate; FLASH → re-plan) before rendering, and every window warning is checked on its window sheet. Then `contact_sheet.py` on the render and a frame either side of each cut, and judge the edit — including against `EDIT-[BUILD]`: every layout on its line. Save rough cut + EDL + report to `08_EDIT`.
...
| 2 | Voice source clips, **at least two** | Kling connector `kling-video-v3_0_omni`, one call per take, `image_1` = step-1 image, 9:16, 1080p, `duration: 10`, `enable_audio: true`, `prefer_multi_shots: false`; G1 = opening script line, G2 = the next, each within the 10s word budget; `VOICE-[CHAR]` first in delivery, identical in every take |
